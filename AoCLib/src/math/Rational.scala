...
extension (x: Long)
  ...
  def %% (y: Long): Rational = Rational(x, y)
  def * (r: Rational): Rational = r * x
  def / (r: Rational): Rational = r.denominator * x %% r.nominator
...
object Rational extends Fractional[Rational]:
  ...
  def apply(n: Long, d: Long): Rational =
    ...
    if n == 0 then new Rational(0, 1)
      ...
      val k = scala.math.signum(d) * gcd(n, d)
  ...
  override def parseString(str: String): Option[Rational] = str.split("/") match
    case Array(n, d) =>
      for
        n <- Numeric.LongIsIntegral.parseString(n)
  ...
  override def abs(x: Rational): Rational = new Rational(scala.math.abs(x.nominator), x.denominator)
